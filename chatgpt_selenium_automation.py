...
class ChatGPTAutomation:
    _instance = None  # Instance holder

    def __new__(cls, chrome_path, chrome_driver_path):
        """Overrides the __new__ method to ensure a single instance."""
        if cls._instance is None:
            cls._instance = super(ChatGPTAutomation, cls).__new__(cls)
            # Proceed with initialization only if the instance is new
            cls._instance.__init__(chrome_path, chrome_driver_path)
        return cls._instance

    def __init__(self, chrome_path, chrome_driver_path):
        """Constructor modified to handle singleton pattern."""
